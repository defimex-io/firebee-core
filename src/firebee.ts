...
export function buyNewLevel(matrix: u64, level: i64): void {
    ...
    assert(msg.amount == levelPrice.getOrDefault(u32(level), U256.ZERO), "invalid price");
    assert(level > 1 && level <= MAX_LEVEL, "invalid level");

    let sendUser = userDB.getUser(msg.sender);

    if (matrix == 1) {
        assert(!sendUser.activeX3Levels[i32(level)], "level already activated");

        if (sendUser.x3Matrix[i32(level - 1)].blocked) {
            sendUser.x3Matrix[i32(level - 1)].blocked = false;
        }

        let freeX3Referrer = findFreeX3Referrer(msg.sender, level);
        sendUser.x3Matrix[i32(level)].currentReferrer = freeX3Referrer;
        sendUser.activeX3Levels[i32(level)] = true;
